...
export async function listApplications(examOccurrenceId?: string, status?: string, page = 1, take = 1000, searchQuery?: string, isWaiting?: boolean): Promise<{ data: ApplicationData[]; meta: { itemCount: number; pageCount: number; hasPreviousPage: boolean; hasNextPage: boolean } }> {
  ...
  if (isWaiting !== undefined) {
    params.append('isWaiting', isWaiting.toString());
  }
