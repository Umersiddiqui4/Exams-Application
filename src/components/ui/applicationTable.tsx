...
export default function ApplicationTable() {
  ...
  const chek = initialExams.find((exam) => exam.id === selectedExam);
  const check = chek ? chek.examType === "AKTs" : false;
  console.log("Check Exam:", check);
  ...
  const generatePdfBlob = async (data: any, images: any) => {
    if (!check) {
      ...
      const doc = <ApplicationPDFCompleteAktApp data={data} />;
      const asPdf = pdf();
      asPdf.updateContainer(doc);
      const blob = await asPdf.toBlob();
      return blob;
                ...
                <div className="flex items-center">
                  ...
                  <span className="font-medium dark:text-slate-200 text-slate-700">
                    ...
                    <span className="text-[#5c347d] dark:text-[#8b5fbf] font-semibold">
                      {applications.find(
                        (exam: any) => exam.examId === selectedExam
                      )?.examName || "N/A"}
                    </span>
                  </span>
                </div>
